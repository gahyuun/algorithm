...
function solution(cacheSize, cities) {
  ...
  for (let i = 0; i < cities.length; i++) {
    ...
    //캐시 hit인지 추적
    const cityIndex = list.getIndex(city);

    if (cityIndex !== null) {
      time += 1;
      // 히트 되었으니까 맨 마지막으로 보내기
      list.removeAt(cityIndex);
      list.insertLast(city);
    } // 캐시가 hit인 것.
    else {
      time += 5;
      list.insertLast(city); // 삽입하고
      if (list.getSize() > cacheSize) {
        list.removeAt(0); // 제일 첫번째꺼 지우기
      }
    }
  }
  return time;
